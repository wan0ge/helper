...
module.exports = async function hokan({ site, input, output, force }) {
  ...
  const existences = force ? [] : await getExistences(input, site);
  let todos = items
    .filter((item) => !exclusions.includes(item.id))
    .filter((item) => force || !existences.includes(item.id))
    .map((item) => ({ site, ...item }));
