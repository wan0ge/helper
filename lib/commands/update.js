...
module.exports = async function update({ input, month, force, siteList }) {
  ...
  const newItems = await items.reduce(async (seqItems, item, idx) => {
    ...
    const sites = await normalizeSites(item.sites).reduce(async (seqSites, site) => {
      ...
      const seasonId = site.season_id || '';
      const begin = await crawler[site.site].getBegin(site.id, site.site, seasonId).catch(() => '');
      ...
      return resultSites.concat({
        ...
        ...(site.video_sn && { video_sn: site.video_sn }),
        ...(site.season_id && { season_id: site.season_id }),
        ...(site.related && site.related.length > 0 && { related: site.related }),
      });
    }, Promise.resolve([]));
